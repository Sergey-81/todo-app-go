...
func TestFilterByDateRange(t *testing.T) {
    tm := NewTaskManager() // Создаем менеджер внутри теста
    
    // Создаем тестовые даты
    now := time.Now()
    testDates := []struct {
        desc       string
        dateOffset int // дней от текущей даты
        wantInclude bool
    }{
        {"Задача до диапазона", -2, false},
        {"Начало диапазона", 1, true},
        {"Середина диапазона", 3, true},
        {"Конец диапазона", 5, true},
        {"После диапазона", 6, false},
    }

    // Добавляем тестовые задачи
    for _, td := range testDates {
        dueDate := now.AddDate(0, 0, td.dateOffset)
        id, _ := tm.AddTask(td.desc, nil)
        
        _, err := tm.UpdateTask(id, UpdateTaskRequest{
            DueDate: &dueDate,
        })
        if err != nil {
    ...
    // Задаем диапазон фильтрации (завтра -> через 5 дней)
    start := now.AddDate(0, 0, 1)
    end := now.AddDate(0, 0, 5)

    // Фильтруем
    filtered := tm.FilterByDateRange(start, end)

    // Проверяем количество
    expectedCount := 3
    if len(filtered) != expectedCount {
        t.Errorf("Ожидалось %d задач в диапазоне, получено %d", expectedCount, len(filtered))
    }

    // Проверяем сортировку по дате
    for i := 1; i < len(filtered); i++ {
        if filtered[i-1].DueDate.After(filtered[i].DueDate) {
            t.Error("Задачи не отсортированы по дате")
            break
        }
    }
}
...
func TestFilterByDateRange_Empty(t *testing.T) {
    ...
    if len(filtered) != 0 {
        t.Errorf("Ожидалось 0 задач, получено %d", len(filtered))
    }
}
